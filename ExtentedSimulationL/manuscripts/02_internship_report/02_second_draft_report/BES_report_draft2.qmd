...
```{r, warning=FALSE, include=TRUE, echo=FALSE, message=FALSE}
#| fig-cap: "Boundary case, true correlation = 0.1, Hi and Hc are both false"
...
orig_dat %>%
  ...
  labs(x = expression(log[10](pBF_ic)), 
       ...
       caption = "Red line: log(1) = 0, 1000 iterations, \n Number of studies (k), within study sample size (n) and reliability (errorsd)", 
       fill = "Error SD") +  
  theme_minimal()
```
...
```{r, warning=FALSE, include=TRUE, echo=FALSE, message=FALSE}
#| fig-cap: "Boundary case, true correlation = 0.8, Hi and Hc are both false"
...
dat_all_es_types %>%
  ...
  labs(x = expression(log[10](pBF_ic)), 
       y = "Number of Studies (k)", 
       caption = "Red line: log(1) = 0, 1000 iterations, \n Number of studies (k), within study sample size (n) and reliability (errorsd)", , fill = "Error SD") +  
  theme_minimal()
```
...
```{r, warning=FALSE, include=TRUE, echo=FALSE, message=FALSE}
#| fig-cap: "Boundary case, Hi and Hc are both false"
#| fig-subcap:
#|   - "True covariance = 0.8"
#|   - "True beta = 0.8"
#| layout-ncol: 2
...
As can be seen in Figure 3a (which shows simulation 2c because this is an extreme case), the covariances behaved exactly as we expected; in simulation 2a-2d, all $log_{10} \text{ } pBF_{ic}$ distributions were nicely centered around 0 (all simulation plots can be found <github/lodewijn>). 

However, when $\beta$ was the effect size of interest, the $log_{10} \text{ } pBF_{ic}$ distributions did not behave as expected (see Figure 3b). All distributions showed the same pattern that was observed in the correlations, with more extreme boundary values resulting in stronger shifts towards the hypothesis that had the largest parameter space. This means that it is unlikely that the shift in evidence can be explained by prior truncation, as this does not happen in $\beta$.
...
For the covariance:
\begin{align}
&Cov(X_{obs} + \epsilon_X,Y_{obs} + X + \epsilon_Y)  \\
...
For the correlation:
\begin{align}
&Cor(X_{obs} + \epsilon_X,Y_{obs} + X + \epsilon_Y) \\
&= \frac{Cov(X_{obs} + \epsilon_X,Y_{obs} + X + \epsilon_Y)}{\sqrt{Var(X + \epsilon_X) \cdot Var(Y + \epsilon_Y)}} \\
...
\begin{align}
&\beta_{obs} = \frac{Cov(X_{obs} + \epsilon_X,Y_{obs} + X + \epsilon_Y)}{Var(X_{obs} + \epsilon_X)} = \frac{\rho}{1 + errorsd^2}
\end{align}
...
To see if attenuation would also play a role using frequentist methods, RMA was used to pool correlation estimates from the k different studies. The mean correlation estimate and SE resulting from this were then passed to `bain` to get one BF, which was repeated 1000 times, to then plot all BFs. 

```{r, warning=FALSE, include=TRUE, echo=FALSE, message=FALSE}
#| fig-cap: "RMA, boundary case, true correlation = 0.1, Hi and Hc are both false"
...
dat_all_es_types %>%
 ...
 labs(x = expression(log[10](BF_rma_ic)), 
  ...
  theme_minimal()
```
Figure 4 shows the $log_{10} \text{ } BF_{ic}$ distributions when first pooling all correlation estimates, and then performing RMA. It stands out that the shift in evidence towards $H_c$ is also visible here. Consistent with the pBFs, with more extreme boundary values such as 0.8, the shift in evidence towards $H_c$ was more extreme in this case. This means that the evidence shift introduced by measurement error persists at the level of the pooled estimate, and pooling correlation estimates via RMA before applying `bain` does not protect against attenuation. 

## Simulation 4: Ordered hypotheses
Given that attenuation of correlation seems to play a role both in Bayesian and frequentist methods, the question of how to deal with this arose. When testing ordered hypotheses, such as $H_i: \rho_1 < \rho_2 < \rho3$, attenuation may not be as big of a problem, if all correlations would have equal measurement error. To see how the pBF would behave in this scenario, we set up a simulation where there were two conditions for $rho$:
 
- Unequal: $rho_1$ = Cor(X,Y) = -0.1, $rho_2$ = Cor(X,Z) = 0, $rho_3$ = Cor(Y,Z) = 0.1, so $H_i$ was true
- Equal: $rho_1 = rho_2 = rho_3$ = 0.1, so $H_i$ and $H_c$ were both

In addition, for the error, three different conditions were tested:

- Equal error at 0
- Equal error at 0.5
- Unqual error, where $\epsilon_X$ = 0.81, $\epsilon_Y$ = 0, $\epsilon_Z$ = 0.5
